...
| 2 — Tool execution | 🔄 ~90% | Done: registry, 9 tools (read + gated write/git/shell/python), sandboxed `run_shell` + `python_exec`, policy enforcement, golden cases. Left: web retrieval, tool replay UX |
...
**Current focus:** Phase 2 close-out — web retrieval; then Phase 4 memory ([#6](https://github.com/xlibraries/agent-hub/issues/6)).
